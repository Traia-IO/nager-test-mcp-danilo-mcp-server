#!/usr/bin/env python3
"""
Nager Test MCP Danilo MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('nager-test-mcp-danilo_mcp')
...
logger.info("="*80)
logger.info(f"Nager Test MCP Danilo MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://date.nager.at")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("Nager Test MCP Danilo MCP Server", host="0.0.0.0")
...
# - NAGER_TEST_MCP_DANILO_API_KEY: Server's internal API key for payment mode
#
...
@require_payment_for_tool(
    price=TokenAmount(
        amount="40000000000000000",  # 0.04 tokens
        asset=TokenAsset(
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="nager-test-mcp-danilo-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "nager-test-mcp-danilo-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Nager Test MCP Danilo MCP Server")
    logger.info("="*80)
