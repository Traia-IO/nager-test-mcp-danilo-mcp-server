...
# Script to build and run the Nager Test MCP Danilo MCP Server locally in Docker
...
IMAGE_NAME="nager-test-mcp-danilo-mcp-server"
CONTAINER_NAME="nager-test-mcp-danilo-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Nager Test MCP Danilo MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Nager Test MCP Danilo MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ Nager Test MCP Danilo MCP Server is running!${NC}"
echo
