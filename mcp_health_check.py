#!/usr/bin/env python
"""
Nager Test MCP Danilo MCP Server Health Check Script

This script properly connects to the Nager Test MCP Danilo MCP server and checks its health by:
...
def check_mcp_server_health(url: str) -> bool:
    """Check if Nager Test MCP Danilo MCP server is healthy"""
    print(f"🔍 Checking Nager Test MCP Danilo MCP server health at {url}")
    ...
    if "error" in result and "session" not in str(result.get("error", "")):
        ...
        if "result" in result and "tools" in result["result"]:
            ...
            if missing_tools:
                ...
                print("✅ All expected Nager Test MCP Danilo tools are available!")
...
def main():
    parser = argparse.ArgumentParser(description="Check Nager Test MCP Danilo MCP Server Health")
    parser.add_argument("--url", default="http://localhost:8000", 
                       help="Nager Test MCP Danilo MCP server URL (default: http://localhost:8000)")
    args = parser.parse_args()
    
    print(f"🚀 Nager Test MCP Danilo MCP Server Health Check")
    print(f"📍 Server URL: {args.url}")
    print(f"📰 Expected tools: example_tool")
    print("="*50)
    
    if check_mcp_server_health(args.url):
        print("\n✅ Nager Test MCP Danilo MCP Server is healthy and responding!")
        return 0
    else:
        print("\n❌ Nager Test MCP Danilo MCP Server health check failed!")
        return 1
